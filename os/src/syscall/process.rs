...
pub fn sys_waitpid(pid: isize, exit_code_ptr: *mut i32) -> isize {
    ...
    if let Some((idx, _)) = pair {
        ...
        // ++++ release child PCB
        // 传入的exit_code_ptr指向应用地址空间 所以需要手动查页表
...
pub fn sys_mmap(_start: usize, _len: usize, _port: usize) -> isize {
    ...
    // 查页表确定[_start,_start+len)确认未被映射
    let mut start=_start;
    let end=start+_len;
    while start < end {
        match inner.memory_set.translate(VirtAddr::from(start).floor()){
            Some(_) => {
                println!("{:?}already mapped.",start);
                return -1;
            }
            None => {
                println!("[before]start:{:?}",start);
                start+=PAGE_SIZE;
    ...
    inner.memory_set.insert_framed_area(VirtAddr::from(_start), VirtAddr::from(end), perm);
...
pub fn sys_munmap(_start: usize, _len: usize) -> isize {
    ...
    println!("before");
    inner.memory_set.remove_area_with_start_vpn(VirtAddr::from(_start).floor().into());
    0
}
