...
## 对本次实验的看法
    - 我这学期刚学完linux操作系统（大三下），虽然我们学校的课程质量一般+架构不同，但还是能找到很多类似的内容，所以理解起来压力不大
    - 对我来说ch3算是一个入门，让我能够上手第二阶段的实验。之前删了又删clone了又clone还是不知道应该如何运行，我觉得没有一个明确的指引。经过ch3之后，知道了测例在哪看；因为要上手改代码所以对整体结构有了更实在的把握。同时做到了一定程度对rust知识的查漏补缺，比如refcell，我当时几乎是学完就忘。
    不过现在我觉得我还是需要在用到某一个知识点的时候回去翻看rust圣经/rust程序设计语言两本书。